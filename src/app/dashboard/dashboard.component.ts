...
export class DashboardComponent implements OnInit {
  ...
  openProfileDialog(): void{
    ...
    this.apiService.fetchUserProfile().subscribe({next: (userdata)=>{
      this.dialog.open(UserProfileComponent,{position: {top: '50px', right: '20px'},hasBackdrop: false ,data:{username: userdata.username, email: userdata.email, onLogout: () => this.logout(), onNavigateToDashboard: () => this.dashboard()}})
