...
export class DashboardComponent implements OnInit {
  ...
  openProfileDialog(): void{
    ...
    this.apiService.fetchUserProfile().subscribe({next: (userdata)=>{
      this.dialog.open(UserProfileComponent,{position: {top: '55px', right: '20px'},maxWidth: '220px',maxHeight:'300px',data:{username: userdata.username, email: userdata.email, onLogout: () => this.logout(), onNavigateToDashboard: () => this.dashboard()}})
